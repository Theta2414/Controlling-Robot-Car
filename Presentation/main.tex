    ...
    \begin{column}{0.48\textwidth}
      \begin{block}{Cấu hình Phần cứng (Hardware)}
        \begin{itemize}
          ...
          \item \textbf{Nguồn:} Pin Li-ion \qty{3.7}{\V} ở tay cầm; Pack pin \qty{7.4}{\V} ở xe (hạ áp qua LM2596 và AMS1117).
        \end{itemize}
      \end{block}
    \end{column}
      ...
      \begin{block}{Cấu hình Phần mềm (Software)}
        \begin{itemize}
          ...
          \item \textbf{Giao tiếp ngoại vi:}
            \begin{itemize}
              \item I2C: Tốc độ \qty{100}{\kHz}.
              \item UART: Baudrate \qty{9600}{\bps}, 8-N-1.
              \item \textbf{PWM: Tần số \qty{10}{\kHz}} trên Timer 2 (êm ái, giảm tiếng rít từ động cơ).
            \end{itemize}
    ...
    \begin{column}{0.31\textwidth}
      \begin{block}{Chi tiết kết nối}
        ...
        \begin{itemize}
          ...
          \item \textbf{Nguồn cấp:} Pin Li-ion \qty{3.7}{\V} kết hợp mạch hạ áp AMS1117-\qty{3.3}{\V} cấp nguồn ổn định.
        \end{itemize}
      \end{block}
    \end{column}
  \end{columns}
    ...
    \begin{column}{0.31\textwidth}
      \begin{block}{Chi tiết kết nối}
        ...
        \begin{itemize}
          ...
          \item \textbf{Nguồn cấp:} Pin Li-ion \qty{7.4}{\V}, hạ áp qua mạch Buck LM2596 và LDO AMS1117-\qty{3.3}{\V}.
        \end{itemize}
      \end{block}
    \end{column}
  \end{columns}
...
\begin{frame}{3. Triển khai — Thuật toán lọc Kalman góc nghiêng}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{Dung hợp cảm biến (Sensor Fusion)}
        \begin{itemize}
          \item \textbf{Gia tốc kế:} Đo góc dựa trên trọng trường, phản hồi ổn định dài hạn nhưng nhạy với rung động (nhiễu cao tần).
          \item \textbf{Con quay hồi chuyển:} Phản ứng nhanh, mượt nhưng bị trôi sai số tĩnh (drift/bias) theo thời gian.
          \item \textbf{Kalman Filter} thực hiện ước lượng tối ưu góc nghiêng thực dựa trên dự báo từ gyro và hiệu chỉnh từ accelerometer.
          \item \textbf{Tham số lọc:} $Q_{\text{angle}} = 0.001$, $Q_{\text{bias}} = 0.003$, $R_{\text{measure}} = 0.03$.
        \end{itemize}
      \end{block}
    \end{column}
    \begin{column}{0.48\textwidth}
...
\begin{frame}{3. Triển khai — Công thức bộ lọc Kalman}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{1. Bước Dự báo (Prediction)}
        \begin{itemize}
          \item \textbf{Hiệu chỉnh bias Gyro:}
            $$\omega_k = \omega_{\text{gyro},k} - b_{k-1}$$
          \item \textbf{Dự báo góc nghiêng:}
            $$\hat{\theta}_k^{-} = \hat{\theta}_{k-1} + \omega_k \Delta t$$
          \item \textbf{Dự báo hiệp phương sai sai số:}
            $$P_{k}^{-} = A P_{k-1} A^T + Q$$
        \end{itemize}
      \end{block}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{block}{2. Bước Cập nhật (Measurement Update)}
        \begin{itemize}
          \item \textbf{Tính sai lệch dự báo (Residual):}
            $$y_k = \theta_{\text{acc}} - \hat{\theta}_k^{-}$$
          \item \textbf{Hệ số Kalman (Kalman Gain):}
            $$K_0 = \frac{P_{00}^{-}}{P_{00}^{-} + R}$$
          \item \textbf{Ước lượng góc tối ưu:}
            $$\hat{\theta}_k = \hat{\theta}_k^{-} + K_0 y_k = (1-K_0)\hat{\theta}_k^{-} + K_0 \theta_{\text{acc}}$$
        \end{itemize}
      \end{block}
    \end{column}
  \end{columns}
...
\begin{frame}{3. Triển khai — Hiệu chuẩn \& Chu kỳ xử lý}
  ...
  \begin{block}{Chu kỳ xử lý và Tần số thực thi}
    \small
    \begin{itemize}
      \item MPU6050 được cấu hình tần số lấy mẫu phần cứng ở mức \qty{100}{\Hz}.
      \item Vòng lặp chính khối phát chạy chu kỳ \textbf{\qty{10}{\ms}} ($f_{\text{loop}} = \qty{100}{\Hz}$) nhờ hàm \texttt{HAL\_Delay(10)}.
    ...
    \begin{column}{0.31\textwidth}
      \begin{block}{Các trạng thái hoạt động}
        ...
        \begin{itemize}
          ...
          \item \texttt{\scriptsize IDLE (vùng chết):} Góc nghiêng tay trong dải $\pm 8^\circ$ (Deadzone), gửi lệnh dừng xe.
          \item \texttt{\scriptsize FORWARD / BACKWARD:} Tay nghiêng quá ngưỡng dọc ($\pm 8^\circ$), gửi lệnh tiến/lùi.
          \item \texttt{\scriptsize LEFT / RIGHT:} Tay nghiêng quá ngưỡng ngang ($\pm 8^\circ$), gửi lệnh rẽ trái/phải.
        \end{itemize}
      \end{block}
    \end{column}
  \end{columns}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.31\textwidth}
      \begin{block}{Nguyên lý hoạt động}
        \footnotesize
        \begin{itemize}
          \item \textbf{Khởi động:} Cấu hình ngắt UART USART1 và cấu hình PWM \qty{10}{\kHz} trên Timer 2.
...
\begin{frame}{4. Thử nghiệm — Kịch bản vận hành thực tế}
  \begin{block}{Thiết lập hệ thống kiểm thử}
    \begin{itemize}
      \item Thiết bị cầm tay: STM32F103 + MPU6050 + Bluetooth Master + Pin Li-ion \qty{3.7}{\V}.
      \item Xe thử nghiệm: Khung xe 4 bánh DC + STM32F103 + Bluetooth Slave + 2x TB6612 + Pin \qty{7.4}{\V}.
    ...
    \begin{column}{0.48\textwidth}
      \begin{block}{\textcolor{MLIOTwhite}{Ưu điểm nổi bật}}
        \begin{itemize}
          ...
          \item Tần số băm xung điều tốc động cơ \textbf{\qty{10}{\kHz}} giúp động cơ chạy cực kỳ êm.
