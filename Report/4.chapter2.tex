...
\hline
2 & Xử lý trung tâm khối phát (tay cầm) & Vi điều khiển STM32\-F103 (lõi Cortex-M3) & Tính toán góc và đóng gói dữ liệu lệnh gửi qua UART & Ref. Manual STM32F103 \cite{stm32-hal} & Sụt áp nguồn phát gây treo hoặc reset vi điều khiển & Sử dụng ổn áp AMS1117-\qty{3.3}{\volt} và thêm tụ lọc nguồn \qty{10}{\micro\farad} gần chân VDD \\
\hline
3 & Giao tiếp không dây nối tiếp & Module Bluetooth HC-05 (Master-Slave) & Truyền nhận gói dữ liệu điều khiển qua UART (Baud \num{9600}) & Datasheet HC-05 & Nhiễu sóng \qty{2.4}{\giga\hertz} và mất kết nối khi vượt khoảng cách cho phép & Cài đặt tính năng dừng an toàn (fail-safe) tự ngắt động cơ khi mất kết nối \\
\hline
...
\hline
5 & Điều khiển công suất động cơ DC & Driver cầu H TB6612FNG và động cơ DC chổi than & Cấp xung PWM điều tốc (Timer 2) và xuất GPIO điều khiển chiều & Datasheet TB6612 & Dòng khởi động lớn gây sụt áp nguồn chính hoặc quá dòng khi kẹt động cơ & Tách nguồn động cơ và vi xử lý, thêm tụ điện \qty{470}{\micro\farad} lọc nguồn \\
\hline
...
\begin{itemize}
    ...
    \item \textbf{Driver cầu H TB6612FNG:} Sử dụng hai module driver cầu H để điều khiển độc lập 4 bánh xe DC. Hệ thống cấu hình 4 kênh xung PWM của Timer 2 (PA0, PA1, PA2, PA3) cùng các chân điều hướng GPIO (PB0, PB1, PB10, PB11, PB13, PB14, PB15, PA8) để kiểm soát tốc độ và chiều quay của từng động cơ.
...
\begin{enumerate}
    ...
    \[ \phi = \mathrm{atan2}(A_y, A_z) \times \frac{180}{\pi} \]
    ...
    \item \textbf{Khối thu}: Module Bluetooth HC-05 (Slave) nhận sóng RF và chuyển tiếp gói dữ liệu đến STM32 khối thu qua UART. Vi điều khiển giải mã gói tin, kiểm tra tính hợp lệ và cập nhật trạng thái hoạt động của xe. Đồng thời, một cơ chế kiểm tra thời gian chờ (Timeout) liên tục giám sát trạng thái kết nối. Nếu quá \qty{0.3}{\second} (\qty{300}{\ms}) không nhận được gói tin mới, hệ thống tự động kích hoạt chế độ Fail-safe để bảo vệ an toàn cho xe.
\end{enumerate}

Cách xử lý tín hiệu và chuyển đổi góc thành tín hiệu điều khiển được trình bày chi tiết ở Chương \ref{chap:mpu-kalman}.
...
\begin{itemize}
    ...
    \item Xử lý dải chết (Deadzone): Nếu góc nghiêng nhỏ hơn \qty{8}{\degree}, coi như tay nằm ngang (Lệnh Dừng). Nếu vượt quá \qty{8}{\degree}, hệ thống bắt đầu tính toán giá trị điều khiển và ánh xạ góc nghiêng sang mức tốc độ PWM tương ứng theo hàm phi tuyến.
    \item Truyền dữ liệu: Đóng gói giá trị Forward và Turn thành chuỗi ký tự ASCII theo định dạng \texttt{"F:\%d T:\%d\textbackslash n"} và gửi ra cổng USART1.
    \item Trì hoãn (Delay): Chờ \qty{10}{\milli\second} trước khi lặp lại chu kỳ mới.
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Khi nhận được gói tin UART hợp lệ kết thúc bằng ký tự xuống dòng \texttt{'\textbackslash n'}: Giải mã chuỗi thành hai giá trị Forward và Turn, đặt lại biến đếm thời gian về 0, tính toán điều tốc vi sai bánh xe trái/phải, cập nhật các chân GPIO hướng quay và chu kỳ nhiệm vụ (Duty Cycle) của xung PWM.
    \end{itemize}
\end{itemize}
...
Để quản lý hành vi và đảm bảo tính thời gian thực cũng như độ an toàn cao nhất, hệ thống điều khiển được thiết kế dựa trên mô hình máy trạng thái finite (Finite State Machine - FSM) riêng biệt cho cả khối phát (tay cầm) và khối thu (xe robot).

\subsubsection{Máy trạng thái khối phát (Transmitter FSM)}
Khối phát sử dụng máy trạng thái tuần tự để quản lý quá trình khởi động, hiệu chuẩn cảm biến và liên tục truyền dữ liệu:
\begin{enumerate}
    \item \textbf{STATE\_INIT (Khởi tạo):} Trạng thái khởi tạo phần cứng ngay sau khi cấp nguồn. Vi điều khiển cấu hình các ngoại vi GPIO, USART1, I2C1 và kiểm tra phản hồi từ cảm biến MPU6050.
    \item \textbf{STATE\_CALIB (Hiệu chuẩn):} Khi MPU6050 sẵn sàng, hệ thống thu thập \num{200} mẫu dữ liệu quán tính khi thiết bị đứng yên nhằm tính toán các giá trị offset của góc nghiêng và sai lệch tĩnh (bias) của gyroscope.
    \item \textbf{STATE\_ACTIVE (Hoạt động \& Truyền tin):} Trạng thái hoạt động chính. Hệ thống chạy chu kỳ \qty{10}{\ms}, liên tục đọc cảm biến, lọc Kalman để ước lượng góc Pitch và Roll, tính toán lệnh điều khiển phi tuyến và gửi chuỗi ký tự qua Bluetooth.
\end{enumerate}

Bảng chuyển đổi trạng thái của khối phát:
\begin{table}[H]
\centering
\renewcommand{\arraystretch}{1.3}
\caption{Bảng chuyển đổi trạng thái (FSM) của khối phát}
\label{tab:tx_fsm_transitions}
\changefontsizes{9pt}
...
\hline
STATE\_INIT & STATE\_CALIB & Khởi tạo phần cứng thành công và cảm biến MPU6050 sẵn sàng & Gửi thông báo Checkpoint qua UART, bắt đầu đếm số mẫu hiệu chuẩn \\
\hline
STATE\_CALIB & STATE\_ACTIVE & Thu thập đủ \num{200} mẫu dữ liệu thành công & Tính toán trung bình cộng offset/bias, lưu giá trị hiệu chuẩn \\
\hline
\end{tabularx}
\end{table}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{Drawio/Transmitter_FSM.drawio.pdf}
    \caption{Sơ đồ máy trạng thái khối phát}
    \label{fig:transmitter-fsm}
\end{figure}

\subsubsection{Máy trạng thái khối thu (Receiver FSM)}
Khối thu sử dụng máy trạng thái để kiểm soát chuyển động và thực hiện cơ chế dừng khẩn cấp (Fail-safe):
\begin{enumerate}
    \item \textbf{STATE\_RUNNING (Trạng thái hoạt động bình thường):} Trạng thái xe kết nối Bluetooth ổn định với tay cầm. Ở trạng thái này, vi điều khiển khối thu giải mã các giá trị điều khiển \(F\) và \(T\) nhận được từ cổng UART, sau đó thực hiện thuật toán vi sai để điều tốc và điều hướng xe thông qua hàm \texttt{Motor\_SetControl(F, T)}.
    \item \textbf{STATE\_FAIL\_SAFE (Trạng thái dừng an toàn):} Trạng thái khẩn cấp kích hoạt ngay lập tức khi mất tín hiệu kết nối Bluetooth giữa khối phát và khối thu quá thời gian quy định (\qty{0.3}{\second} hoặc \qty{300}{\ms}). Trạng thái này có độ ưu tiên cao nhất, lập tức tắt xung PWM (\qty{0}{\percent}) và ngắt dòng động cơ bằng hàm \texttt{Motor\_Stop()} nhằm bảo vệ xe an toàn, đồng thời chỉ cho phép chuyển sang trạng thái \textbf{STATE\_RUNNING} khi nhận lại được gói tin Bluetooth hợp lệ từ khối phát.
\end{enumerate}

Bảng chuyển đổi trạng thái của khối thu:
\begin{table}[H]
\centering
\renewcommand{\arraystretch}{1.3}
...
\textbf{Trạng thái hiện tại} & \textbf{Trạng thái tiếp theo} & \textbf{Điều kiện chuyển trạng thái} & \textbf{Hành động thực hiện} \\
\hline
STATE\_RUNNING & STATE\_FAIL\_SAFE & Mất kết nối Bluetooth quá \qty{0.3}{\second} (\qty{300}{\ms}) & Tắt xung PWM (\qty{0}{\percent}), gọi hàm dừng động cơ xe \\
\hline
STATE\_FAIL\_SAFE & STATE\_RUNNING & Nhận được gói tin UART mới hợp lệ kết thúc bằng ký tự \texttt{'\textbackslash n'} & Cập nhật thời điểm nhận tin và gọi hàm điều khiển động cơ \\
\hline
\end{tabularx}
\end{table}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{Drawio/Receiver_FSM.drawio.pdf}
