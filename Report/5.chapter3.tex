...
\qty{\pm 2}{\g}  & \qty{16384}{LSB/\g} \\ \hline
\qty{\pm 4}{\g}  & \qty{8192}{LSB/\g} \\ \hline
\qty{\pm 8}{\g}  & \qty{4096}{LSB/\g} \\ \hline
\qty{\pm 16}{\g} & \qty{2048}{LSB/\g} \\ \hline
\end{tabular}
...
\hline
\textbf{Phạm vi đo} & \textbf{Độ nhạy} \\ \hline
\qty{\pm 250}{\degree\per\second}  & \qty{131}{LSB/(\degree/\second)} \\ \hline
\qty{\pm 500}{\degree\per\second}  & \qty{65.5}{LSB/(\degree/\second)} \\ \hline
\qty{\pm 1000}{\degree\per\second} & \qty{32.8}{LSB/(\degree/\second)} \\ \hline
\qty{\pm 2000}{\degree\per\second} & \qty{16.4}{LSB/(\degree/\second)} \\ \hline
\end{tabular}
...
Mức \qty{\pm 250}{\degree\per\second} được lựa chọn vì hệ thống chủ yếu cần theo dõi sự thay đổi góc nghiêng của thiết bị điều khiển, không yêu cầu đo các chuyển động quay có vận tốc góc quá lớn. Việc sử dụng thang đo nhỏ giúp tăng độ nhạy của phép đo trong phạm vi hoạt động của hệ thống.
...
\begin{equation}
Pitch\approx\qty{0}{\degree}
\end{equation}
\begin{equation}
Roll\approx\qty{0}{\degree}
\end{equation}
...
Vòng lặp chính được thiết lập với khoảng thời gian xử lý mục tiêu khoảng \qty{10}{\ms}. Do đó, tần số xử lý của vòng lặp có thể được ước tính xấp xỉ:
\begin{equation}
f_{\text{loop}}
\approx
\frac{1}{0.01}
...
Như vậy, cảm biến và vòng lặp chính đều được thiết lập ở cùng tần số. Trong chương trình hiện tại, dữ liệu cảm biến được đọc và đưa vào Kalman Filter với tần số xử lý mục tiêu khoảng \qty{100}{\hertz}, trong khi giá trị $\Delta t$ được sử dụng trong bộ lọc vẫn được xác định dựa trên thời gian thực tế giữa hai lần cập nhật.
...
\begin{equation}
\qty{-60}{\degree} \leq \theta \leq \qty{60}{\degree}
\end{equation}
và được ánh xạ sang:
\begin{equation}
-80 \leq u \leq +80
\end{equation}
trong đó $u$ là giá trị điều khiển (giới hạn ở mức \qty{80}{\percent} công suất tối đa để bảo vệ cơ cấu chấp hành).

Hệ thống đồng thời sử dụng vùng chết:
\begin{equation}
\qty{-8}{\degree} < \theta < \qty{8}{\degree}
\end{equation}
...
với điều kiện:
\begin{equation}
\qty{8}{\degree} \leq |\theta| \leq \qty{60}{\degree}
\end{equation}
...
\begin{table}[H]
    ...
    % Cấu hình cột 2 dạng S với định dạng số gồm 1 dấu và 2 chữ số nguyên (+2.0)
    \begin{tabular}{c S[table-format=+2.0]}
        \toprule
        \textbf{Góc nghiêng} & {\textbf{Giá trị điều khiển}} \\ % Bọc tiêu đề trong cặp { } để siunitx hiểu là text
        \midrule
        \qty{-60}{\degree} & -80 \\
        \qty{-34}{\degree} & -20 \\
        \qty{0}{\degree}   &  0 \\
...
Sau khi nhận được giá trị điều khiển $F$ ($control_{\text{forward}}$) và $T$ ($control_{\text{turn}}$) từ khối phát, khối thu tiến hành thuật toán phối trộn vi sai để xác định tốc độ của các bánh xe bên trái ($V_{\text{left}}$) và bên phải ($V_{\text{right}}$):
\begin{equation}
...
\end{equation}
Trong đó, hàm $\text{clamp}(val, \num{-100}, \num{100})$ thực hiện giới hạn giá trị tốc độ bánh xe trong phạm vi từ \num{-100} đến \num{100} (tương ứng chu kỳ nhiệm vụ xung PWM từ \qty{-100}{\percent} đến \qty{100}{\percent}). Hướng quay của từng bánh xe sẽ được quyết định bởi dấu của $V_{\text{left}}$ và $V_{\text{right}}$ thông qua các chân GPIO điều khiển chiều của driver cầu H, và tốc độ bánh xe được điều khiển bằng độ rộng xung PWM tỉ lệ với giá trị tuyệt đối của chúng.
