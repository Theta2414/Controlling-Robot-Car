...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.55\textwidth]{mpu6050.jpg}
...
Trong hệ thống điều khiển robot bằng chuyển động nghiêng, cần xác định được hướng và mức độ nghiêng của thiết bị điều khiển để chuyển đổi thành lệnh di chuyển cho robot. Để thực hiện nhiệm vụ này, hệ thống sử dụng cảm biến quán tính MPU6050.
...
MPU6050 hỗ trợ nhiều mức thang đo cho accelerometer, bao gồm:
...
Trong hệ thống, gyroscope có thể được cấu hình ở nhiều mức thang đo khác nhau, bao gồm:
...
Phương pháp này cho phép gyroscope phản ứng nhanh đối với chuyển động. Tuy nhiên, do tồn tại sai số bias, sai số góc sẽ tích lũy theo thời gian.
...
Khi thành phần $\mathbf{a}_{\text{motion}}$ lớn, góc được suy ra từ accelerometer có thể sai lệch đáng kể so với góc thực.

Do đó, trong hệ thống, góc accelerometer được sử dụng như một nguồn thông tin hiệu chỉnh cho thuật toán sensor fusion.
...
Để giải quyết hạn chế của từng cảm biến, hệ thống sử dụng Kalman Filter để thực hiện sensor fusion giữa accelerometer và gyroscope.
...
Kalman Filter là một thuật toán ước lượng trạng thái của một hệ thống từ các phép đo có nhiễu.
...
Như vậy, hệ thống không chỉ ước lượng góc mà còn theo dõi bias của gyroscope trong quá trình hoạt động.

Hai bộ lọc độc lập được sử dụng cho Pitch và Roll.
...
Ma trận này biểu diễn mức độ không chắc chắn của trạng thái ước lượng.

Trong quá trình dự đoán, độ không chắc chắn được cập nhật dựa trên:
...
Phương trình trên cho thấy kết quả cuối cùng là sự kết hợp giữa prediction và measurement.

Khác với phép trung bình thông thường, trọng số của mỗi nguồn dữ liệu được xác định dựa trên độ không chắc chắn của hệ thống.
...
Nhờ vậy, Kalman Filter có khả năng điều chỉnh ước lượng bias trong quá trình hoạt động thay vì chỉ sử dụng một giá trị cố định.

Điều này đặc biệt hữu ích đối với việc hạn chế drift của gyroscope.
...
\subsection{Ảnh hưởng của $Q_{\text{angle}}$}

Khi tăng $Q_{\text{angle}}$, hệ thống giả định rằng mô hình prediction có nhiều bất định hơn. Do đó, measurement có xu hướng có ảnh hưởng lớn hơn đến kết quả.

Ngược lại, khi giảm $Q_{\text{angle}}$, bộ lọc có xu hướng tin prediction nhiều hơn.

\subsection{Ảnh hưởng của $Q_{\text{bias}}$}

Tăng $Q_{\text{bias}}$ cho phép bộ lọc thích nghi nhanh hơn với sự thay đổi của bias gyroscope.

Giảm $Q_{\text{bias}}$ khiến bias được xem là ổn định hơn và thay đổi chậm hơn.

\subsection{Ảnh hưởng của $R_{\text{measure}}$}

Nếu tăng $R_{\text{measure}}$, measurement từ accelerometer được xem là nhiều nhiễu hơn. Bộ lọc sẽ giảm mức độ tin tưởng vào accelerometer và duy trì nhiều hơn thông tin từ prediction.

Nếu giảm $R_{\text{measure}}$, accelerometer được xem là đáng tin cậy hơn và ảnh hưởng mạnh hơn đến kết quả.

Do đó, tuning các tham số $Q$ và $R$ cần được thực hiện dựa trên đặc tính thực tế của cảm biến cũng như yêu cầu về độ mượt và tốc độ đáp ứng của  obot.

% =========================================================
% DT
% =========================================================

\section{Tính toán khoảng thời gian $\Delta t$}
\label{sec:dt}
...
Vòng lặp chính được thiết lập với khoảng thời gian xử lý mục tiêu khoảng $10$ ms. Do đó, tần số xử lý của vòng lặp có thể được ước tính xấp xỉ:
...
\begin{equation}
f_{\text{MPU6050}}=100\ \text{Hz}
\end{equation}

Như vậy, cảm biến và vòng lặp chính đều được thiết lập ở cùng tần số. Trong chương trình hiện tại, dữ liệu cảm biến được đọc và đưa vào Kalman Filter với tần số xử lý mục tiêu khoảng $100$ Hz, trong khi giá trị $\Delta t$ được sử dụng trong bộ lọc vẫn được xác định dựa trên thời gian thực tế giữa hai lần cập nhật.
...
Trong đó $u$ là giá trị điều khiển.
...
Mối quan hệ giữa góc và giá trị điều khiển được mô tả:
...
Bảng sau minh họa quá trình ánh xạ:

\begin{table}[H]
    \centering
    \caption{Ánh xạ góc nghiêng sang giá trị điều khiển}
    \label{tab:angle-control}
    \begin{tabular}{c c}
        \toprule
        \textbf{Góc nghiêng} & \textbf{Giá trị điều khiển} \\
        \midrule
        $-45^\circ$ & $-100$ \\
        $-22.5^\circ$ & $-50$ \\
        $0^\circ$ & $0$ \\
...
\section{Giải thích các thuật ngữ tiếng Anh}
\label{sec:terminology}

Bảng dưới đây trình bày các thuật ngữ tiếng Anh được sử dụng trong báo cáo cùng với nghĩa tiếng Việt và ý nghĩa của chúng trong hệ thống.
...
\endhead
    ...
    Accelerometer &
    Cảm biến gia tốc &
    Đo gia tốc theo ba trục $X$, $Y$, $Z$ và được sử dụng để tính góc Pitch và Roll. \\
    \hline

    Gyroscope &
    Cảm biến con quay hồi chuyển &
    Đo vận tốc góc theo ba trục $X$, $Y$, $Z$ và cung cấp thông tin về chuyển động quay. \\
    \hline

    Raw data &
    Dữ liệu thô &
    Dữ liệu trực tiếp thu được từ MPU6050 trước khi chuyển đổi và hiệu chuẩn. \\
    \hline

    Calibration &
    Hiệu chuẩn &
    Quá trình xác định và loại bỏ độ lệch của cảm biến trước khi sử dụng dữ liệu. \\
    \hline

    Bias &
    Độ lệch &
    Sai lệch của Gyroscope khi cảm biến đứng yên, được ước lượng trong quá trình hiệu chuẩn. \\
    \hline

    Offset &
    Độ lệch ban đầu &
    Giá trị góc ban đầu được xác định trong quá trình hiệu chuẩn và sử dụng làm mốc tham chiếu. \\
    \hline

    Pitch &
    Góc nghiêng Pitch &
    Góc nghiêng được sử dụng để xác định mức độ điều khiển tiến/lùi của robot. \\
    \hline

    Roll &
    Góc nghiêng Roll &
    Góc nghiêng được sử dụng để xác định mức độ điều khiển rẽ của robot. \\
    \hline

    Yaw &
    Góc quay Yaw &
    Góc quay quanh trục $Z$; không được sử dụng trong quá trình điều khiển hiện tại. \\
    \hline

    Kalman Filter &
    Bộ lọc Kalman &
    Bộ lọc kết hợp thông tin từ Accelerometer và Gyroscope để ước lượng góc nghiêng. \\
    \hline

    Prediction &
    Dự đoán &
    Bước dự đoán góc tiếp theo dựa trên góc hiện tại và vận tốc góc từ Gyroscope. \\
    \hline

    Measurement &
    Phép đo &
    Giá trị góc được xác định từ dữ liệu Accelerometer. \\
    \hline

    Measurement Update &
    Cập nhật phép đo &
    Bước hiệu chỉnh giá trị dự đoán bằng thông tin đo từ Accelerometer. \\
    \hline

    Process noise &
    Nhiễu quá trình &
    Thành phần biểu diễn mức độ không chắc chắn của mô hình trong quá trình dự đoán. \\
    \hline

    Measurement noise &
    Nhiễu phép đo &
    Thành phần biểu diễn mức độ nhiễu của phép đo góc từ Accelerometer. \\
    \hline

    Covariance &
    Hiệp phương sai &
    Đại lượng biểu diễn mức độ không chắc chắn của trạng thái ước lượng trong Kalman Filter. \\
    \hline

    Kalman gain &
    Hệ số Kalman &
    Hệ số xác định mức độ ảnh hưởng của phép đo mới đến giá trị ước lượng sau khi cập nhật. \\
    \hline

    State &
    Trạng thái &
    Đại lượng được Kalman Filter ước lượng, trong hệ thống là góc nghiêng và độ lệch Gyroscope. \\
    \hline

    State estimate &
    Ước lượng trạng thái &
    Giá trị trạng thái được ước lượng sau quá trình prediction và measurement update. \\
    \hline

    Residual &
    Sai số phép đo &
    Độ chênh lệch giữa giá trị đo từ Accelerometer và giá trị góc được dự đoán bởi Kalman Filter. \\
    \hline
    
    Drift &
    Hiện tượng trôi &
    Sai lệch tích lũy theo thời gian, thường xuất hiện trong dữ liệu Gyroscope. \\
    \hline

    Sampling rate &
    Tần số lấy mẫu &
    Số lần cảm biến thực hiện lấy dữ liệu trong một giây. \\
    \hline

    DLPF &
    Bộ lọc thông thấp số &
    Bộ lọc được sử dụng để giảm ảnh hưởng của nhiễu tần số cao trong dữ liệu cảm biến. \\
    \hline

    Deadzone &
    Vùng chết &
    Khoảng góc nhỏ quanh $0^\circ$ trong đó giá trị điều khiển được đặt bằng $0$. \\
    \hline

    Saturation &
    Giới hạn / bão hòa &
    Giới hạn góc hoặc giá trị điều khiển trong phạm vi cho phép. \\
    \hline

    Mapping &
    Ánh xạ &
    Quá trình chuyển đổi góc nghiêng thành giá trị điều khiển. \\
    \hline

    Control signal &
    Tín hiệu điều khiển &
    Giá trị được tạo ra từ góc nghiêng để xác định mức độ điều khiển robot. \\
    \hline

    Forward control &
    Điều khiển tiến/lùi &
    Giá trị điều khiển được tạo ra từ góc Pitch sau Kalman. \\
    \hline

    Turn control &
    Điều khiển rẽ &
    Giá trị điều khiển được tạo ra từ góc Roll sau Kalman. \\
    \hline

    Loop &
    Vòng lặp &
    Chu kỳ xử lý được thực hiện lặp lại trong chương trình. \\
    \hline

    $\Delta t$ / Time interval &
    Khoảng thời gian &
    Khoảng thời gian giữa hai lần cập nhật liên tiếp của Kalman Filter. \\
    \hline

    Rate &
    Vận tốc góc &
    Tốc độ thay đổi góc được lấy từ Gyroscope. \\
    \hline

    Sensitivity &
    Độ nhạy &
    Hệ số chuyển đổi giá trị số của cảm biến sang đại lượng vật lý tương ứng. \\
    \hline

    LSB &
    Đơn vị số &
    Đơn vị biểu diễn giá trị số đầu ra của cảm biến. \\
    \hline

    I2C &
    Giao tiếp I2C &
    Giao thức giao tiếp được sử dụng để trao đổi dữ liệu giữa STM32 và MPU6050. \\
    \hline

    WHO\_AM\_I &
    Thanh ghi nhận dạng thiết bị &
    Thanh ghi được sử dụng để kiểm tra và xác nhận thiết bị MPU6050 đang giao tiếp. \\
    \hline
    \caption{Giải thích các thuật ngữ tiếng Anh sử dụng trong hệ thống}
    \label{tab:english-vietnamese}
\end{longtable}
...
\bibitem{stm32-hal}
STMicroelectronics,
\textit{Description of STM32F1 HAL and Low-Layer Drivers},
STM32F1 HAL User Manual.

\bibitem{kalmanfilter-vn}
KalmanFilter.net,
\textit{Kalman Filter - Bộ lọc Kalman},
\url{https://kalmanfilter.net/VI/default_vi.aspx}.
